...
if [[ -n "$FUNCTION_ID" ]]; then
  yc serverless function version create \
    ...
    --source-path="$ZIP" \
    --environment "SITE_URL=${SITE_URL:-https://grillzcustoms.ru},TELEGRAM_FETCH_TIMEOUT_MS=${TELEGRAM_FETCH_TIMEOUT_MS:-25000}${TELEGRAM_BOT_TOKEN:+,TELEGRAM_BOT_TOKEN=$TELEGRAM_BOT_TOKEN}${GROQ_API_KEY:+,GROQ_API_KEY=$GROQ_API_KEY}${OPENROUTER_API_KEY:+,OPENROUTER_API_KEY=$OPENROUTER_API_KEY}${GEMINI_API_KEY:+,GEMINI_API_KEY=$GEMINI_API_KEY}${ADMIN_CHAT_ID:+,ADMIN_CHAT_ID=$ADMIN_CHAT_ID}${START_PHOTO_URL:+,START_PHOTO_URL=$START_PHOTO_URL}"
else
  yc serverless function version create \
