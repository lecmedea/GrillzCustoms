...
if [[ -n "$FUNCTION_ID" ]]; then
  yc serverless function version create \
    ...
    --source-path="$ZIP" \
    --environment "SITE_URL=${SITE_URL:-https://grillzcustoms.ru},TELEGRAM_FETCH_TIMEOUT_MS=${TELEGRAM_FETCH_TIMEOUT_MS:-25000}${TELEGRAM_BOT_TOKEN:+,TELEGRAM_BOT_TOKEN=$TELEGRAM_BOT_TOKEN}${IVASYA_PROXY_URL:+,IVASYA_PROXY_URL=$IVASYA_PROXY_URL}${IVASYA_PROXY_TOKEN:+,IVASYA_PROXY_TOKEN=$IVASYA_PROXY_TOKEN}${SILICONFLOW_API_KEY:+,SILICONFLOW_API_KEY=$SILICONFLOW_API_KEY}${SILICONFLOW_MODEL:+,SILICONFLOW_MODEL=$SILICONFLOW_MODEL}${SILICONFLOW_BASE_URL:+,SILICONFLOW_BASE_URL=$SILICONFLOW_BASE_URL}${OPENAI_COMPAT_API_KEY:+,OPENAI_COMPAT_API_KEY=$OPENAI_COMPAT_API_KEY}${OPENAI_COMPAT_MODEL:+,OPENAI_COMPAT_MODEL=$OPENAI_COMPAT_MODEL}${OPENAI_COMPAT_BASE_URL:+,OPENAI_COMPAT_BASE_URL=$OPENAI_COMPAT_BASE_URL}${GROQ_API_KEY:+,GROQ_API_KEY=$GROQ_API_KEY}${GROQ_MODEL:+,GROQ_MODEL=$GROQ_MODEL}${OPENROUTER_API_KEY:+,OPENROUTER_API_KEY=$OPENROUTER_API_KEY}${OPENROUTER_MODEL:+,OPENROUTER_MODEL=$OPENROUTER_MODEL}${GEMINI_API_KEY:+,GEMINI_API_KEY=$GEMINI_API_KEY}${GEMINI_MODEL:+,GEMINI_MODEL=$GEMINI_MODEL}${MISTRAL_API_KEY:+,MISTRAL_API_KEY=$MISTRAL_API_KEY}${MISTRAL_MODEL:+,MISTRAL_MODEL=$MISTRAL_MODEL}${CEREBRAS_API_KEY:+,CEREBRAS_API_KEY=$CEREBRAS_API_KEY}${CEREBRAS_MODEL:+,CEREBRAS_MODEL=$CEREBRAS_MODEL}${CLOUDFLARE_API_TOKEN:+,CLOUDFLARE_API_TOKEN=$CLOUDFLARE_API_TOKEN}${CLOUDFLARE_ACCOUNT_ID:+,CLOUDFLARE_ACCOUNT_ID=$CLOUDFLARE_ACCOUNT_ID}${CLOUDFLARE_MODEL:+,CLOUDFLARE_MODEL=$CLOUDFLARE_MODEL}${HF_TOKEN:+,HF_TOKEN=$HF_TOKEN}${HF_MODEL:+,HF_MODEL=$HF_MODEL}${ADMIN_CHAT_ID:+,ADMIN_CHAT_ID=$ADMIN_CHAT_ID}${START_PHOTO_URL:+,START_PHOTO_URL=$START_PHOTO_URL}"
else
  yc serverless function version create \
